...
class Findings {
  ...
  async makeApiRequest(endpoint){
   ...
   async getAllRepositoryFiles(){
    ...
    while(true){
      currentPage++
      let pageData = await (await this.makeApiRequest(`projects/7/repository/tree?recursive=true&per_page=500&page=${currentPage}`)).json()
   ...
   async getFile(path){
    return await (await this.makeApiRequest(`projects/7/repository/files/${encodeURIComponent(path)}?ref=master`)).json()
   }
   ...
   async getAllFileData(){
    ...
    let issueCount = 0;
      ...
      fileDataObject && this.fileData.push({
